...
import { read, read64, write, copy, getAsIndexPath, getPkgVersion } from './utils.js'
 ...
 * @param {string} options.quiet — If true, `htmlc` will not log to the console.
 *
...
export const htmlc = async ({
  ...
  try {
    ...
    if (watch) {
        ...
        Promise.all(
          files
            ...
            .map(async htmlFile => {
              ...
              log(`    Writing ${styles.path(outPath)}...`)
            })
        )
      ]
      if (other) {
      ...
      let message = ` ${styles.success('Finished')} — ${compiledCount} file${compiledPlural} compiled`
      if (other) {
        message += `, ${copiedCount} file${copiedPlural} copied.`;
      } else {
        message += '.';
      }
      log(message)
